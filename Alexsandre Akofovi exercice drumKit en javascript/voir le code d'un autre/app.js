...
function addTodo(event) {
    ...
    let completedButton;
    let trashButton;
    let buttons = [completedButton, trashButton];
    let diffText = ['<i class="fas fa-check"> </i>', '<i class="fas fa-trash"> </i>'];
    let icons = ["completed-btn", "trash-btn"];

    for (let i = 0; i < buttons.length; ++i) {
        // CHECK MARK BUTTON ===> We can use both: .createElement() or innerHTML()
        buttons[i] = document.createElement("button");
        buttons[i].innerHTML = diffText[i];
        buttons[i].classList.add(icons[i]);
        newTodo.appendChild(buttons[i]);
    }
...
function saveLocalTodos(todo) {
    ...
    if (localStorage.getItem("todos").value !== undefined) {
